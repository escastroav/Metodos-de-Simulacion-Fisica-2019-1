...
int main(void){
  ...
  Planeta[2].Inicie(x1*cos(M_PI/3.0),x1*sin(M_PI/3.0),0
		    ,-V1*sin(M_PI/3.0),V1*cos(M_PI/3.0),0
		    ,0.005,30);
  
  for(t=tdibujo=0;t<20*T;t+=dt,tdibujo+=dt){
    for(int i = 0; i < N; i++)
      cout<<(cos(omega*t)*Planeta[i].Getx() + sin(omega*t)*Planeta[i].Gety())
	  <<"\t"
	  <<(-sin(omega*t)*Planeta[i].Getx() + cos(omega*t)*Planeta[i].Gety())
	  <<"\t";
    cout<<" "<<endl;
    /*if(tdibujo>T/1000){
      InicieCuadro();
