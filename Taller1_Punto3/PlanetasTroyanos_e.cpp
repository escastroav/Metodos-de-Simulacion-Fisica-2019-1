...
int main(){
  ...
  double T1=T/sqrt(27*m1/(4*m0)), T2=T/(1-(27*m1/(8*m0)));
  ...
  for(t=tdibujo=0;t<20*T;t+=dt,tdibujo+=dt){
    ...
    //    std::cout<<x_rot<<" "<<y_rot<<" "<<t<<std::endl;//Coord del sistema que rota con júpiter
    //    std::cout<<Planeta[2].Getx()<<"\t"<<Planeta[2].Gety()<<std::endl;//Coord de Júpiter
