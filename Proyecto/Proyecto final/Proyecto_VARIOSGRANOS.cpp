...
const double g = 9.8, k = 1e4, Gamma = 5e2, kcundall = 1e308, MU = 0.4, surfTension=27.5e-1;//surfTension es el gamma de la fuerza capilar
const double lx = 0, ly = 110;

const int N = 100, NumP = 100;
...
void Colisionador::CalculeFuerzaEntre(Cuerpo & Molecula1, Cuerpo & Molecula2, vector3D & ele, char & Contacto , double dt){
  ...
  if(-s<0.2){//Fuerza capilar
    ...
    double F_capilar_max=-k*sqrt(0.5);//La fuerza de Hertz si se los granos atravesaran unos a otros hasta 1/4 de su tamaño
    double F_cap=0;
    ...
    double Th2 =Theta2(-s);
    //Fuerza Capilar
    F_cap=-2*M_PI*R*R*surfTension*Th2/std::abs(-s-R*Th2);
    if(F_cap<F_capilar_max) F_cap=F_capilar_max;
    ComponenteFn+=F_cap;
...
void InicieAnimacion(void){
  ...
  std::cout<<"set output 'capi_2307v5_27.5.gif'"<<std::endl;
  std::cout<<"unset key"<<std::endl;
  std::cout<<"set xrange[-105:"<<lx+105<<"]"<<std::endl;
...
int main(){
  ...
  //---------- Paredes ---------------
  double Mpared=100*m0, Rpared = 1.0;
  //  Cuerpo MolPared;
