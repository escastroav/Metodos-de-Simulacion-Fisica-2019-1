...
void Colisionador::CalculeTodasLasFuerzas(std::vector<Cuerpo> & Molecula,double dt){
  ...
  for(i=NumP;i<Molecula.size();i++){
    Molecula[i].AgregueFuerza(Gvector*Molecula[i].m);
  }
  
  for(i=NumP;i<Molecula.size();i++){
    for(k=0;k<NumP;k++){
      CalculeFuerzaEntre(Molecula[i],Molecula[k],ele[i][k],EstabaEnContacto[i][k],dt);
    }
...
int main(){
  ...
  for(int i = 0; i < NumP; i++){
    ...
    Molecula[i].Inicie(-50 + 2*i*Rpared,-Rpared,0,0,0,0,0,0,Mpared,Rpared);
  }
  ...
  for(t=tdibujo=0,tcrear=1;t<T;t+=dt,tdibujo+=dt,tcrear++){
    ...
    for(i=NumP;i<Molecula.size();i++) Molecula[i].Mueva_r(dt,E);
    
    Newton.CalculeTodasLasFuerzas(Molecula,dt);
    for(i=NumP;i<Molecula.size();i++) Molecula[i].Mueva_v(dt,L1);
    
    for(i=NumP;i<Molecula.size();i++) Molecula[i].Mueva_r(dt,X);
    
    Newton.CalculeTodasLasFuerzas(Molecula,dt);
    for(i=NumP;i<Molecula.size();i++) Molecula[i].Mueva_v(dt,L);
    
    for(i=NumP;i<Molecula.size();i++) Molecula[i].Mueva_r(dt,XE);
