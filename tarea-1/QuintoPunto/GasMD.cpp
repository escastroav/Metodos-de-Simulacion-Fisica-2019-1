...
public:
  ...
  vector3D GetPared(void){return Pared;}; //Inline
  bool PreguntaSiChoca(void){return Choca;}; //Inline
  friend class Colisionador;
};
void Cuerpo::Inicie(double x0,double y0,double z0,double Vx0,double Vy0,double Vz0,double m0,double R0,bool Choca0){
  r.cargue(x0,y0,z0); V.cargue(Vx0,Vy0,Vz0); m=m0; R=R0; Choca = Choca0;
}
...
int main(int argc, char** argv){
  Cuerpo Molecula[N+4];
  Colisionador Newton;
  vector3D Vantes[N], Vdespues[N];
  double t,tdibujo,dt=1.0e-3;
  Crandom ran64(1);
  double KT=atof(argv[1]);
  double m0=1,R0=2.5;
  double y,yprom,intesidad=0;
  int i,j;
  ...
  for(i=0;i<Nx;i++){
    for(j=0;j<Ny;j++){
      ...
      Molecula[i*Ny+j].Inicie(x0,y0,0,V0x,V0y,0,m0,R0,false);
    }
  }
  ...
  for(t=tdibujo=0;t<T;t+=dt,tdibujo+=dt){
    ...
    Newton.CalculeTodasLasFuerzas(Molecula);
    for(i=0;i<N;i++){
      if(Molecula[i].PreguntaSiChoca()){
	Vantes[i].cargue(Molecula[i].GetVx(),Molecula[i].GetVy(),Molecula[i].GetVz());
      }
    }
    for(i=0;i<N;i++) Molecula[i].Mueva_V(dt,Coeficiente1);
    for(i=0;i<N;i++){
      if(Molecula[i].PreguntaSiChoca()){
	Vdespues[i].cargue(Molecula[i].GetVx(),Molecula[i].GetVy(),Molecula[i].GetVz());
      }
    }
    for(i=0;i<N;i++){
      if(Molecula[i].PreguntaSiChoca()){
	intesidad+= m0*abs((Vdespues[i]-Vantes[i])*Molecula[i].GetPared());
      }
    }
  ...
  cout<<intesidad/(T*(2*Lx+2*Ly))<<endl;
